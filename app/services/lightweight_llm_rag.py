...
class LightweightLLMRAG:
    ...
    def _generate_prompt_by_length(self, response_length: str, context: str, conflict_info: str, query: str) -> str:
        ...
        if response_length == "short":
            return f"""You are a legal research assistant. Provide BRIEF, direct answers based on the provided legal documents.
...
Legal Question: {query}"""
            ...
            return f"""You are a legal research assistant focused on delivering comprehensive, detailed information based strictly on the provided legal documents.
...
Legal Question: {query}"""
            ...
            return f"""You are a legal research assistant focused on delivering direct, actionable information based strictly on the provided legal documents.

CORE PRINCIPLES:
1. Direct Answers: Provide immediate, actionable answers to the specific question asked.
2. Document-Based Only: Ground all responses strictly in the provided context.
3. Concise Format: Use bullet points and clear structure for readability.
4. Specific Citations: Include exact article/section references for verification.

FORMATTING RULES:
- Use simple bullet points with dashes (-) only
- Reference articles as "Article 41" without special formatting
- Keep formatting minimal and professional
- Maximum 1-2 paragraphs
...
Legal Question: {query}"""
    ...
    async def _generate_llm_response(self, query: str, relevant_docs: List[Dict[str, Any]], conflicts: List[Dict[str, Any]] = None) -> str:
        ...
        try:
            ...
            # Get response length setting and generate appropriate prompt
