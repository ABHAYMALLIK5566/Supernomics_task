...
class QueryComplexityDetector:
    """Advanced query complexity detection with legal domain awareness"""
    
    def __init__(self):
        self.complexity_analyzer = QueryComplexityAnalyzer()
        
        # Legal-specific complexity patterns
        self.legal_complexity_patterns = {
            ...
            "causal_chain": [
                r"because.*?therefore",
                r"due\s+to.*?as\s+a\s+result",
                r"leads\s+to.*?consequently"
            ],
            "multi_document": [
                r"across.*?documents",
                r"throughout.*?chapters",
                r"in\s+all.*?sections",
